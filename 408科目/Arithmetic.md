...
5. `Prim算法`
   ...
   2. 过程如下图所示
      ...
      4. 以此类推，直至图中所有的顶点都并入T，得到的T就是最小生成树。此时T中必然有n-1条边
         ...
         ![截屏2022-09-06 12.25.53](../images/%E6%88%AA%E5%B1%8F2022-09-06%2012.25.53.png)
...
6. Kruskal算法
   ...
   2. 构造最小生成树的**过程**
      ...
      3. 以此类推，直至T中所有顶点都在一个连通分量上
         ...
         ![image-20220908141432133](../images/image-20220908141432133.png)
...
3. `Dijkstra算法求单源最短路径问题`
   ...
   5. **边上带有负权值时**，Dijkstra算法并不适用。若允许边上带有**负权值**，则在与S（已求得最短路径的顶点集，归入S内的结点的最短路径不再变更）内某点a以负边相连的点b确定其最短路径时，其最短路径长度加上这条负边的权值结果可能小于a原先确定的最短路径长度，而此时a在Dijkstra算法下是**无法更新**的
      ...
      ![image-20220908152827112](../images/image-20220908152827112.png)
...
2. 有向无环图是描述含有**公共子式**的表达式的有效工具，例如表达式可以用二叉树表示，有一些相同的**子表达式**，在二叉树中，它们也重复出现。若利用有向无环图，则可实现对**相同子式**的共享，从而节省存储空间
   ...
   ![image-20220908165437145](../images/image-20220908165437145.png)
...
3. 对一个AOV网进行拓扑排序的算法有很多，比较常用的方法的步骤是
   ...
   3. 重复1和2直到**当前的AOV网为空**或**当前网中不存在无前驱的顶点**为止。后一种情况说明有向图中必然存在环
      ...
      ![image-20220908170756506](../images/image-20220908170756506.png)
...
1. B树（**多路平衡查找树**），B树中所有结点的孩子个数的**最大值**称为**B树的阶**，通常用**m**表示。一棵**m**阶B树可以是**空树**，或者满足以下条件的**m**叉树
   ...
   4. 所有**非叶结点**的结构如下
      ...
      ![image-20220913101509813](../images/image-20220913101509813.png)
...
2. B树是**所有结点的平衡因子均等于0**的多路平衡查找树
   ...
   ![image-20220913102355938](../images/image-20220913102355938.png)
...
5. `B树的插入`
      ...
      1. 在B树中，每个非失败结点的关键字个数都在区间[$\lceil m/2 \rceil$-1, m-1]内
         ...
         2. 插入后检查被插入结点内关键字的个数，当插入后的结点关键字个数大于m-1时，必须对结点进行分裂
            ...
            1. **分裂**的方法是
               ...
               ![image-20220913142851984](../images/image-20220913142851984.png)
   ...
   3. `B树的删除`
      ...
      2. 当被删除关键字**k**不在终端结点（最底层非叶结点）中时，可以用**k**的前驱（或后继）**$k^1$** 来替代**k**，然后在相应的结点中删除**$k^1$**，关键字**$k^1$**必定落在某个终端结点中，则转换成了**被删关键字在终端结点中**的情形
         ...
         ![image-20220913143502131](../images/image-20220913143502131.png)
      ...
      3. 当**被删关键字在终端结点（最底层非叶结点）中时**，有下列三种情况
         ...
         3. `兄弟不够借`。若被删除关键字所在结点删除前的关键字个数$= \lceil m/2 \rceil -1$，且此时与该结点相邻的左、右兄弟结点的关键字个数均$=\lceil m/2 \rceil -1$，则将关键字删除后与左（或右）兄弟结点及双亲结点中的关键字进行合并
            ...
            ![image-20220913150437273](../images/image-20220913150437273.png)
...
3. **m阶的B+树**与**m阶的B树**的主要差异如下
   ...
   ![image-20220913154336400](../images/image-20220913154336400.png)
...
1. 假设在排序过程中，待排序表**L\[1...n]**在某次排序过程中的某一时刻状态如下
   ...
   ![image-20220913172144566](../images/image-20220913172144566.png)
   ...
   ![image-20220913172449084](../images/image-20220913172449084.png)
...
2. **基本思想**
   ...
   ![image-20220913175839734](../images/image-20220913175839734.png)
...
1. **基本思想**：从后往前两两比较相邻元素的值，若为逆序，则交换它们，直到序列比较完
   ...
   ![image-20220913193821184](../images/image-20220913193821184.png)
...
4. 堆排序需要解决**两**个问题
   ...
   1. 如何将无序序列构造成初始堆
      ...
      ![image-20220913203217206](../images/image-20220913203217206.png)
...
3. 过程
   ...
   ![image-20220913210427652](../images/image-20220913210427652.png)

   ![image-20220913210450290](../images/image-20220913210450290.png)

   ![image-20220913210505677](../images/image-20220913210505677.png)
